...
namespace :deploy do
  ...
  if File.exist?("/etc/init.d/unicorn_#{deploy_secret(:full_app_name)}")
    puts "/etc/init.d/unicorn_#{deploy_secret(:full_app_name)} found. Trying to restart unicorn server"
